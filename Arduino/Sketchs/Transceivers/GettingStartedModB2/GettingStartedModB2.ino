...
void loop() {
  ...
  if ( role == 0 )
  {
    ...
    unsigned int data_receiver;
    if ( radio.available()) {
      // Variable for the received timestamp
      while (radio.available()) {                                   // While there is data ready
        radio.read( &data_receiver, sizeof(unsigned int) );             // Get the payload
      }

      radio.stopListening();                                        // First, stop listening so we can talk
      radio.write( &data_receiver, sizeof(unsigned int) );              // Send the final one back.
